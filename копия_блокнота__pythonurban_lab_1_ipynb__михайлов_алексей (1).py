...
def task_1(list_):  # FIXME заменить название переменной list на list_    
    return [ (item[0].upper()+item[1:]) for item in list_ ]
...
def task_2(list_, filter_population) :     
    return [item for item in list_ if item['population'] > filter_population]
...
def task_3(list, list2):    
    return [n for n in {n for n in list if n in list2}]

fisrt_group = ["Москва", "Москва", 'Владивосток', "Санкт-Петербург"]
second_group = ['Новосибирск', "Ижевск", "Санкт-Петербург", "Москва", "Москва",]

 # TODO Написать функцию task_3

#common_cities =sorted( task_3(fisrt_group , second_group) )# TODO вызвать функцию и передать в неё значения fisrt_group и second_group

print( sorted( task_3(fisrt_group , second_group) ) )  # ['Санкт-Петербург', 'Москва']
...
def task_4(list_ , city):     
    return [item.lower() for item in list_].count(city.lower())
...
print(task_4(list_cities, find_city))  # 4
...
def landscaping_index(list):     
    return {item["territory_name"]: sum(item["green_zones"]) / item["territory_area"] for item in list}
